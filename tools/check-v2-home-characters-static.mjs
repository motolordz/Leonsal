...
const homeJs = fs.readFileSync('js/v2-home.js', 'utf8');
...
assert(landing.includes('class="landing-playground" data-home-play-mode="football"'), 'Landing page should expose the child-facing sports/play scene');
for (const mode of ['football', 'hoops', 'hopscotch', 'calm']) {
  assert(landing.includes(`data-home-play="${mode}"`), `Landing play scene missing mode: ${mode}`);
}
